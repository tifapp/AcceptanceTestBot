...
impl<'v, Value, View: SlackView, MakeView: Fn(&'v Value) -> View> IfLet<'v, Value, View, MakeView> {
    pub fn some(value: Option<&'v Value>, make_view: MakeView) -> Self {
        Self { value, make_view }
    }
}

impl<'v, Value, View: SlackView, MakeView: Fn(&'v Value) -> View> SlackView
    for IfLet<'v, Value, View, MakeView>
{
