...
test(\"Bob\", async () => {
  const testCase = new RoswaalTestCase(\"Bob\", TestActions.beforeLaunch)
  // C
  testCase.appendAction(TestActions.d)
  await roswaalClient.run(testCase)
})
";
            let test_case_code = read_string(
            ...
            assert!(test_case_code == expected_test_case_code_1 || test_case_code == expected_test_case_code_2);
            Ok(())
        })
        .await.unwrap();
    }
