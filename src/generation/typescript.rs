...
testCases.appendResult(await testCase.run())
})
";
...
fn test_case_test_block_start(name: &str) -> String {
    format!("\
test(\"{}\", async () => {{
  const testCase = new TestCase(\"{}\", TestActions.beforeLaunch)
",
        name,
        name
    )
...
test(\"B\", async () => {
  const testCase = new TestCase(\"B\", TestActions.beforeLaunch)
  // Johnny is signed in
...
test(\"A\", async () => {
  const testCase = new TestCase(\"A\", TestActions.beforeLaunch)
  // Johnny is signed in
  testCase.appendAction(TestActions.ensureJohnnyIsSignedIntoHisAccount)
